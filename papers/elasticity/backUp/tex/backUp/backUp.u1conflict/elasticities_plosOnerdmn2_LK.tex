...
However, defining stocks as data rich or data poor based purely on the availability of catch and effort data obscures the fact that considerable uncertainty 
...
measures. These reference points can be used as targets, for example $B_{MSY}$, or limits, for example, $F_{crash}$.
...
As such the study is not modelled on one specific species of fish.
...
\begin{equation} K = a \times L^{b\prime} \end{equation}
...
effects altering the relationship between weight and eggs produced \cite{pérez2012study}.
...
Growth, natural mortality, proportion mature and selectivity-at-age for the stock are shown in Figure~\ref{fig:stock}.
While the equilibrium values for projections over a range of fishing mortalitydynamics are shown in Figure~\ref{fig:brp}, 
along with the MSY, $F_{0.1}$ and $F_{crash}$ reference points. 
...
if $B$ $\geq$ $B_{MSY}$ and $F$ $\leq$ $F_{MSY}$; yellow otherwise. I.e. the red quadrant refers to an overfished stock subject to overfishing, 
green to a stock which is neither overfished  or subject to overfishing and the yellow to a stock which is either overfished stock or subject to overfishing.
...
Figures~\ref{fig:elasssb} and \ref{fig:elasf}. The vertical line indicates the boundary between the red and green quadrants and the horizontal line where the 
value of the elasticity is 0, i.e. where varying a parameter has no effect on the measure of interest.

The plots allows several important questions to be addressed, i.e. what is the relative impact of the different biological processes and parameter on the 
estimates of stock status and exploitation? Does the impact depend on the status of the stock so that knowledge of some parameters and processes is more important 
when a stock is depleted than when it is within safe biological limits. Also are F reference points for example more robust than those based on SSB and are
some reference points more robust to uncertainty in key processes than others? Answering these questions will allow choice of robust target and limit
reference points.

Inspection of the elasticities of relative SSB (Figure~\ref{fig:elasssb}) shows that the elasticities of SSB relative to 
$SSB_{MSY}$ and $SSB_{F0.1}$ are broadly similar. For all three reference points the natural mortality parameters $M1$ and $M2$ have the biggest 
proportional effect. The next most important parameter is $a1$ the selectivity parameter for age at full selection. 
...
less so relative to $SSB_{Fcrash}$.The other processes (growth and maturity) have similar impacts to each other; the most important parameters are $K$,
...
\cite{houde1989subtleties}.\cite{houde2002mortality} also suggest a single mortality rate, however points out the various processes that can serieally 
