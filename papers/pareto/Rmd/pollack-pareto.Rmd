 ...
 mseSBTD<-function(
  ...
  fn<-function(spp,cv,nyr,ctrl,srDev,k1k2){
    ...
    mse=mseSBTD(om,eq,
                ...
                u_deviances =uDev,
                start  =108,end=140,
                nyrs   =nyr,lag=0,maxF=refpts(eq)["msy","harvest"]*1.5)
...
mseD<-foreach(i=seq(dim(tar)[1])), 
 ...
 mseSBTD<-function(
  ...
  cpueFn=ssb,
  lag   =0,
  maxF  =c(refpts(eq)["msy","harvest"]*1.5)){
  ...
  fn<-function(spp,cv,nyr,ctrl,srDev,k1k2){
    ...
    set.seed(1235)
    uDev =FLife:::rlnoise(dim(om)[6],FLQuant(0,dimnames=dimnames(iter(fbar(om),1))),cv,b=0.0)
    mse=mseSBTD(om,eq,
                control=k1k2,
                sr_deviances=srDev,
                u_deviances =uDev,
