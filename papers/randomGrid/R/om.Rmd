...
par=model.frame(lh)
par=FLPar(apply(lh,1,mean))
load(file.path(dirDat,"turbot.RData"))
par=cbind(par,FLPar(apply(lh,1,mean)))
...
```{r, eqRun3}
mFn=function(wt,params){
    res=wt%^%params["m2"] 
    res=params["m1"]%*%res
    res} 

eqs=list("Gislason-BevHolt"=lhEql(par,spwn=0),
         "Lorenzen-BevHolt"=lhEql(rbind(par,propagate(FLPar(m1=3,m2=-0.288),7)),spwn=0,m=mFn),
         "Gislason-Ricker" =lhEql(par,spwn=0,sr="ricker"),
         "Lorenzen-Ricker" =lhEql(rbind(par,propagate(FLPar(m1=3,m2=-0.288),7)),spwn=0,m=mFn,sr="ricker"))

save(eqs,file="/home/laurence/Desktop/sea++/mydas/project/tasks/task4/data/eqs.RData") 
