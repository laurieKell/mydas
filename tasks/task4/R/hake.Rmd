...
opts_chunk$set(comment   =NA, 
               ...
               fig.height=10,
               cache     =TRUE, 
               fig.path  ="../tex/om/hake-",
               cache.path="../cache/om/hake/")
...
#dirMy="/home/laurence/Desktop/sea++/mydas/project/tasks/task4"
...
```{r, lh}
lh=lhPar(FLPar(linf=93.3,k=0.41,t0=-0.2,l50=25))
...
```{r, eqRun, fig.width=8,fig.height=6}
eq1=iter(eq,1)
refpts(eq1)=refpts(eq1)[c("msy","crash")]
fbar(eq1)=fbar(eq1)/max(fbar(eq1))*refpts(eq1)["crash","harvest"]
plot(eq1)+theme_bw()  
```
...
om=propagate(as(eq,"FLStock"),nits)
...
plot(FLQuants(om, 
  ...
  geom_hline(aes(yintercept=1),col="red")+theme_bw() 
```
