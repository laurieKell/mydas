---
title: "title: Commercial catch and survey data for use in MSY proxies estimation"
subtitle: "Razors"
author: "L Kell"
...
opts_chunk$set(cache     =!TRUE,
               ...
               fig.height=10,
               fig.width =10,
               fig.path  ="tex/task_5_plan",
               cache.path="cache/task_5_plan/")
...
Task 5 of the MYDAS project <https://github.com/laurieKell/mydas> potentially requires commercial/Survey inputs to estimate the current status of the case-study stocks. 
The available data sources within the EU are either country specific, or from collective sources such as DATRAS <http://www.ices.dk/marine-data/data-portals/Pages/DATRAS.aspx> 
for survey data and the STECF <https://stecf.jrc.ec.europa.eu/data-dissemination> for commercial data. As part of tasks 1-2 these data have already been acquired and subset on a stock basis for the MYDAS
stocks of interest. Also included are estimates obtained from life-history experiments.
...
ggplot(subset(cpue_surv, year >1992), aes(as.factor(year), cpue))+
  ...
  theme_bw()+theme(text = element_text(size=8,face="bold"),       panel.grid.major = element_blank(),       panel.grid.minor = element_blank(),       strip.background = element_blank(),       axis.text=element_text(size=8),       axis.title.x = element_text(face="bold", size=8),       axis.title.y = element_text(face="bold", size=8, angle=90),       strip.text.x = element_text(size = 8,face="bold"))+theme(legend.position="none",axis.text.x=element_text(size=7,angle = -90, hjust = 0))+facet_wrap(~speciesgp, scale="free_y")+
  expand_limits(y=0)+
...
geom_boxplot(fill="#56B4E9")+facet_wrap(~speciesgp, scale="free_y")+ expand_limits(y=0)+
...
ggplot(commctch, aes(as.factor(year),tottonnes/1000))+geom_bar(stat="identity", fill="blue")+
  facet_wrap(~speciesgp, scale="free_y")+theme_bw()+theme(text = element_text(size=8,face="bold"),       panel.grid.major = element_blank(),       panel.grid.minor = element_blank(),       strip.background = element_blank(),       axis.text=element_text(size=8),       axis.title.x = element_text(face="bold", size=8),       axis.title.y = element_text(face="bold", size=8, angle=90),       strip.text.x = element_text(size = 8,face="bold"))+theme(axis.text.x=element_text(size=7,angle = -90, hjust = 0))
```
...
ggplot(subset(nstations3, year >2000), aes(year,totabun))+geom_line()+geom_point()+expand_limits(y=0)+theme_bw()+ylab("Abundance in tonnes")+
  stat_summary(fun.data=mean_cl_normal) + facet_wrap(~speciesgp, scale="free_y")+
  geom_smooth(method='lm',formula=y~x, colour="red") +theme(axis.text.x=element_text(size=7,angle = -90, hjust = 0))
```
...
## The plan - simulation test the below
