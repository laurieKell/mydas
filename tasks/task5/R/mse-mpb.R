...
mse2=mseMPB2(om,eq,mp,start=55,ftar=0.5,srDev=srDev,uDev=uDev,selDev=selDev)

scen=expand.grid(stock=c("turbot","lobster","ray","pollack","razor","brill","sprat"),
                  ftar=c(1.0,0.7),btrig=c(0.5,0.6))
...
for (i in seq(dim(scen)[1])){
  ...
  mpb=rbind(mpb,cbind(scen=i,res))
  
  save(mpb,file="/home/laurence/Desktop/sea++/mydas/tasks/task5/data/mpb.RData")}
