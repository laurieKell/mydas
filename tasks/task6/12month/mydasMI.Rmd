...
+ **<span style="color:brown">Case Studies</span>** based on Economic value; Importance; Sensitivity to the impacts of fishing; Available data.
...
+ **<span style="color:purple">Management Strategy Evaluation</span>** to ensure robustness to uncertainty 
...
**WKLIFE** is therefore testing alternative rules that use **<span style="color:blue">$MSY$</span>** proxy reference points of the form 
...
ggplot(dat)+
  ...
  geom_path(aes(year,`50%`,col=Gain))+
  geom_path(aes(year,data,col=.id,group=paste(iter,.id,spp,qname)),linetype="23"
            ,data=subset(smry,iter%in%c(12)&spp=="sprat"))+
  facet_grid(qname~Gain,scale="free")+
  theme_bw()+
  theme(legend.position="bottom")+ylab("")+xlab("Year")
```
